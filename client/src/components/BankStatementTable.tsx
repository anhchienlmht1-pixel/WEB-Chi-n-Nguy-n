...
import { fetchBankStatement } from "../utils/bankData";

// Full 341-row raw statement export (scripts/export-bank-data.py) — every
// line item from the source Excel's per-bank sheet, not just the 14
// curated ratios in BankFundamentals' KPI table. Values are whatever the
// sheet stored for that row (mostly Tỷ VNĐ, some already fractions for
// ratio rows), so this renders raw numbers rather than guessing a unit.
export default function BankStatementTable({
  ...
  const { data, error, loading } = usePolling(() => fetchBankStatement(symbol), [symbol]);
