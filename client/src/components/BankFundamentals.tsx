...
import type { BankMetricKey, BankPeriodData } from "../types/bank";
...
const CHART_GROUPS: { title: string; keys: BankMetricKey[]; format: "percent" | "money" }[] = [
  { title: "Quy mô: Cho vay & Tiền gửi khách hàng", keys: ["loans", "deposits"], format: "money" },
  { title: "Khả năng sinh lời: ROE & ROA", keys: ["roe4q", "roe", "roa4q", "roa"], format: "percent" },
  { title: "Biên lãi & chi phí vốn: NIM / YOEA / COF", keys: ["nim", "yoea", "cof"], format: "percent" },
  { title: "Thanh khoản: LDR / LLR / CASA", keys: ["ldr", "llr", "casa"], format: "percent" },
...
export default function BankFundamentals({ symbol }: { symbol: string }) {
  const [periodType, setPeriodType] = useState<PeriodType>("quarter");
  const [showStatement, setShowStatement] = useState(false);
  const { data, error, loading } = usePolling(() => fetchBankData(symbol), [symbol]);
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900/40">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 p-4 dark:border-slate-800">
        <div>
          <h3 className="text-base font-bold text-slate-900 dark:text-slate-100">Cơ bản (Ngân hàng)</h3>
      ...
      {!loading && data && (
        <>
            ...
            {CHART_GROUPS.map((group) => {
              ...
              return (
                <BankMetricLineChart
                  ...
                  lines={lines}
                  formatValue={group.format === "percent" ? formatPercentTick : formatMoneyTick}
                />
              );
            })}
          </div>
          ...
          <div className="border-t border-slate-200 p-4 dark:border-slate-800">
            ...
            {showStatement && (
              <div className="mt-4">
                <BankStatementTable symbol={symbol} periodType={periodType} />
              </div>
            )}
          </div>
        </>
...
function KpiTable({ periodData, showYoy }: { periodData: BankPeriodData; showYoy: boolean }) {
  const n = periodData.periods.length;
  const latest = n - 1;
  const prior = n - 2;
  const yearAgo = n - 5; // 4 quarters back from latest
