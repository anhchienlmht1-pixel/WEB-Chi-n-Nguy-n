...
import EquityBarLineChart from "./EquityBarLineChart";

// Bar (absolute value) + line (growth %) combo panels, one shared Quý/Năm
// toggle governing all four — matching the layout used by VN brokerage
...
//
// Revenue/profit are two line items off the same KQKD report, and
// assets/equity off the same CDKT report — each report is fetched once
// and handed to both of its panels. Each panel still falls back
// independently (renders nothing) if its own line item isn't found, so
// one missing metric doesn't hide the other.
export default function StockDashboard({ symbol }: { symbol: string }) {
  ...
  const { data: cdkt, error: cdktError, loading: cdktLoading } = usePolling(
    () => fetchFinancials(symbol, "CDKT", periodType),
    [symbol, periodType]
  );
  ...
  return (
    <div>
      <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
        <h3 className="text-sm font-semibold text-slate-900 dark:text-slate-100">Cơ bản doanh nghiệp</h3>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <AssetsBarLineChart data={cdkt ?? null} loading={cdktLoading} error={cdktError} />
        <EquityBarLineChart data={cdkt ?? null} loading={cdktLoading} error={cdktError} />
      </div>
    </div>
  );
}
