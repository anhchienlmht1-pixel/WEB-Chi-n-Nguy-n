...
import { DEFAULT_BACKTEST_PARAMS, runRsiAtrBacktest, type BacktestParams, type BacktestResult } from "../utils/backtest";
...
// Client-side backtest of a single reference strategy — RSI-oversold entry,
// ATR-based stop-loss/take-profit exit — run entirely on price history
// already fetched from /api/history, no separate backtest endpoint. Ported
// from a user-supplied Python reference (same entry/exit logic, same
// SL-checked-before-TP tie-break).
  ...
  return (
    <div className="mx-auto max-w-[1400px] px-4 py-6">
      ...
      <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
        Chiến lược tham khảo: vào lệnh khi RSI vào vùng quá bán, thoát bằng Stop Loss / Take Profit tính theo ATR.
      </p>
      ...
      <p className="mt-6 text-[11px] text-slate-400 dark:text-slate-500">
        Backtest lịch sử, không phải khuyến nghị đầu tư — hiệu suất quá khứ không đảm bảo kết quả tương lai. Lệnh vào và
        thoát tính theo giá đóng/cao/thấp mỗi phiên, không tính phí giao dịch, trượt giá hay thuế.
      </p>
    </div>
  );
}
